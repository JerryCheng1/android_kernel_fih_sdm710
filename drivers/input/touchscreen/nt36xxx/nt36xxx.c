 ...
 *
 * $Revision: 28982 $
 * $Date: 2018-05-31 11:17:54 +0800 (週四, 31 五月 2018) $
 *
...
static int dsi_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
extern void touch_selftest_nvt(void);
extern int selftest_result_read_nvt(void);
extern void touch_tpfwver_read_nvt(char *);
extern void touch_tpfwimver_read(char *fw_ver);
extern void touch_fwupgrade(int);
extern void touch_fwupgrade_read(char *);
extern void touch_vendor_read_nvt(char *);
extern unsigned int touch_double_tap_read_nvt(void);
extern int touch_double_tap_write_nvt(unsigned int enable);
extern int tp_probe_success;

struct i2c_client * i2c_connect_client_nvt = NULL;
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	if (keycode > 0) {
		...
		NVT_LOG("Report key up!!\n");
		input_report_key(ts->input_dev, keycode, 0);
		input_sync(ts->input_dev);
		NVT_LOG("Report key down!!\n");
	}
}
#endif
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//FIH register drm_notifier instead of fb_notifier -- start
	ts->dsi_panel_notif.notifier_call = dsi_panel_notifier_callback;
	ret = msm_drm_register_client(&ts->dsi_panel_notif);
	if(ret) {
		NVT_ERR("Unable to register drm_notifier : %d", ret);
		goto err_register_fb_notif_failed;
	}
	///FIH register drm_notifier instead of fb_notifier -- end
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	...
	//FIH touch -- start
	tp_probe_success = 1;
	touch_cb.touch_selftest = touch_selftest_nvt;
	touch_cb.touch_selftest_result = selftest_result_read_nvt;
	...
	touch_cb.touch_vendor_read = touch_vendor_read_nvt;
...
//FIH add dsi notifier callback -- start
static int dsi_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	int blank;
	struct msm_drm_notifier *evdata = data;
	struct nvt_ts_data *ts = container_of(self, struct nvt_ts_data, dsi_panel_notif);

	if (!evdata || (evdata->id != 0))
		return 0;

	if (ts && (event == MSM_DRM_EARLY_EVENT_BLANK)) {
		blank = *(int *)(evdata->data);
		if (blank == MSM_DRM_BLANK_UNBLANK) {
			NVT_LOG("%s: receives EARLY_BLANK:UNBLANK\n", __func__);
		} else if (blank == MSM_DRM_BLANK_POWERDOWN) {
			NVT_LOG("%s: receives EARLY_BLANK:POWERDOWN\n", __func__);
			nvt_ts_suspend(&ts->client->dev);
		} else {
			NVT_LOG("%s: receives wrong data EARLY_BLANK:%d\n", __func__, blank);
		}
	}

	if (ts && event == MSM_DRM_EVENT_BLANK) {
		blank = *(int *)(evdata->data);
		if (blank == MSM_DRM_BLANK_POWERDOWN) {
			NVT_LOG("%s: receives BLANK:POWERDOWN\n", __func__);
		} else if (blank == MSM_DRM_BLANK_UNBLANK) {
			NVT_LOG("%s: receives BLANK:UNBLANK\n", __func__);
			nvt_ts_resume(&ts->client->dev);
		} else {
			NVT_LOG("%s: receives wrong data BLANK:%d\n", __func__, blank);
		}
	}
	return 0;
}
//FIH add dsi notifier callback -- end

//FIH touch -- start
void touch_tpfwver_read_nvt(char *fw_ver)
{
	//nvt_get_fw_info();
...
int touch_double_tap_write_nvt(unsigned int enable)
{
	...
	NVT_LOG("%s: gdouble_tap_enable_nvt = enable = %d\n", __func__, gdouble_tap_enable_nvt);
	return 0;
}
